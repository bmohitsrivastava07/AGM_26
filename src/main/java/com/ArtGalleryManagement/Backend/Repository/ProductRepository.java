...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    default Page<Product> findByTitleContainingWithLogging(@RequestParam("title") String title, Pageable pageable){
    	logger.info("Searching for Products by title containing: {}",title);
    	return findByTitleContaining(title,pageable);
    }
    
    default Page<Product> findByCategoryWithLogging(@RequestParam("category") String category, Pageable pageable){
    	logger.info("Searching for products by category: {}",category);
    	return findByCategory(category,pageable);
    }
    
    default List<Product> findProductsByProductIdsWithLogging(@Param("product_ids") List<Long> productIds){
    	logger.info("Searching for products by productIds: {}",productIds);
